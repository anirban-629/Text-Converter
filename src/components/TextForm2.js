...
export default function TextForm2(props) {
  ...
  const upperCase = () => {
    ...
    props.alrt('success','! Text converted in uppercase')
  };
  
  const lowerCase = () => {
    setText(text.toLowerCase());
    props.alrt('success','! Text converted in lowercase')
  };
  ...
  const addUnderScore = () => {
    ...
    let unds = abc.join("_");
    setText(unds);
    props.alrt('success','! Underscore added')
  };
  ...
  const findWord = () => {
    ...
    for (let i = 0; i < words.length; i++) {
      if (finding.value === words[i]) {
        ...
        props.alrt('warning','! Oops..! Word Not Found')
      }
    }
  };
